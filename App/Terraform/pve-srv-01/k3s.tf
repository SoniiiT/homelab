# k3s-soniiit-etcd01
resource "proxmox_vm_qemu" "k3s_soniiit_etcd01" {

    # VM General Settings
    target_node = "pve-srv-01"
    vmid = "0"
    name = "k3s-soniiit-etcd01"
    tags = "ubuntu,kubernetes"
    desc = "Node 01 Server for Kubernetes Cluster"
    automatic_reboot = false

    # The destination resource pool for the new VM
    pool = "K3s"

    # VM Advanced General Settings
    onboot = true
    boot = "order=virtio0;net0"
    bootdisk = "virtio"
    scsihw = "virtio-scsi-pci"

    # VM OS Settings
    full_clone = true
    clone = "ubuntu-server-24-04"
    bios = "seabios"

    # VM System Settings
    agent = 1

    # VM CPU Settings
    cores = 1
    sockets = 1
    cpu = "host"

    # VM Memory Settings
    memory = 4096

    # VM Network Settings
    network {
        bridge = "vmbr0"
        model  = "virtio"
    }

    # VM Disk Settings Cloud-Init
    disks {
        ide {
            ide0 {
                cloudinit {
                    storage = "VM-Storage"
                }
            }
        }
        virtio {
            virtio0 {
                disk {
                    size               = 32
                    storage            = "VM-Storage"
                }
            }
        }
    }

    # VM Cloud-Init Settings
    os_type = "cloud-init"

    # (Optional) IP Address and Gateway
    ipconfig0 = "ip=192.168.178.10/24,gw=192.168.178.1"

    # (Optional) DNS Servers
    nameserver = "192.168.178.3"

    # (Optional) Default User
    ciuser = "soniiit"

    # (Optional) Add your Public SSH KEY
    sshkeys = "ssh-rsa  "
}

# k3s-soniiit-cp01
resource "proxmox_vm_qemu" "k3s_soniiit_cp01" {

    # VM General Settings
    target_node = "pve-srv-01"
    vmid = "0"
    name = "k3s-soniiit-cp01"
    tags = "ubuntu,kubernetes"
    desc = "Node 01 Server for Kubernetes Cluster"
    automatic_reboot = false

    # The destination resource pool for the new VM
    pool = "K3s"

    # VM Advanced General Settings
    onboot = true
    boot = "order=virtio0;net0"
    bootdisk = "virtio"
    scsihw = "virtio-scsi-pci"

    # VM OS Settings
    full_clone = true
    clone = "ubuntu-server-24-04"
    bios = "seabios"

    # VM System Settings
    agent = 1

    # VM CPU Settings
    cores = 2
    sockets = 1
    cpu = "host"

    # VM Memory Settings
    memory = 16384

    # VM Network Settings
    network {
        bridge = "vmbr0"
        model  = "virtio"
    }

    # VM Disk Settings Cloud-Init
    disks {
        ide {
            ide0 {
                cloudinit {
                    storage = "VM-Storage"
                }
            }
        }
        virtio {
            virtio0 {
                disk {
                    size               = 32
                    storage            = "VM-Storage"
                }
            }
            virtio1 {
                disk {
                    size               = 128
                    storage            = "VM-Storage"
                }
            }
        }
    }

    # VM Cloud-Init Settings
    os_type = "cloud-init"

    # (Optional) IP Address and Gateway
    ipconfig0 = "ip=192.168.178.11/24,gw=192.168.178.1"

    # (Optional) DNS Servers
    nameserver = "192.168.178.3"

    # (Optional) Default User
    ciuser = "soniiit"

    # (Optional) Add your Public SSH KEY
    sshkeys = "ssh-rsa  "
}

# k3s-soniiit-node01
resource "proxmox_vm_qemu" "k3s_soniiit_node01" {

    # VM General Settings
    target_node = "pve-srv-01"
    vmid = "0"
    name = "k3s-soniiit-node01"
    tags = "ubuntu,kubernetes"
    desc = "Node 02 Server for Kubernetes Cluster"
    automatic_reboot = false

    # The destination resource pool for the new VM
    pool = "K3s"

    # VM Advanced General Settings
    onboot = true
    boot = "order=virtio0;net0"
    bootdisk = "virtio"
    scsihw = "virtio-scsi-pci"

    # VM OS Settings
    full_clone = true
    clone = "ubuntu-server-24-04"
    bios = "seabios"

    # VM System Settings
    agent = 1

    # VM CPU Settings
    cores = 2
    sockets = 1
    cpu = "host"

    # VM Memory Settings
    memory = 16384

    # VM Network Settings
    network {
        bridge = "vmbr0"
        model  = "virtio"
    }

    # VM Disk Settings Cloud-Init
    disks {
        ide {
            ide0 {
                cloudinit {
                    storage = "VM-Storage"
                }
            }
        }
        virtio {
            virtio0 {
                disk {
                    size               = 32
                    storage            = "VM-Storage"
                }
            }
            virtio1 {
                disk {
                    size               = 128
                    storage            = "VM-Storage"
                }
            }
        }
    }

    # VM Cloud-Init Settings
    os_type = "cloud-init"

    # (Optional) IP Address and Gateway
    ipconfig0 = "ip=192.168.178.12/24,gw=192.168.178.1"

    # (Optional) DNS Servers
    nameserver = "192.168.178.3"

    # (Optional) Default User
    ciuser = "soniiit"

    # (Optional) Add your Public SSH KEY
    sshkeys = "ssh-rsa  "
}

# k3s-soniiit-node02
resource "proxmox_vm_qemu" "k3s_soniiit_node02" {

    # VM General Settings
    target_node = "pve-srv-01"
    vmid = "0"
    name = "k3s-soniiit-node02"
    tags = "ubuntu,kubernetes"
    desc = "Node 03 Server for Kubernetes Cluster"
    automatic_reboot = false

    # The destination resource pool for the new VM
    pool = "K3s"

    # VM Advanced General Settings
    onboot = true
    boot = "order=virtio0;net0"
    bootdisk = "virtio"
    scsihw = "virtio-scsi-pci"

    # VM OS Settings
    full_clone = true
    clone = "ubuntu-server-24-04"
    bios = "seabios"

    # VM System Settings
    agent = 1

    # VM CPU Settings
    cores = 2
    sockets = 1
    cpu = "host"

    # VM Memory Settings
    memory = 16384

    # VM Network Settings
    network {
        bridge = "vmbr0"
        model  = "virtio"
    }

    # VM Disk Settings Cloud-Init
    disks {
        ide {
            ide0 {
                cloudinit {
                    storage = "VM-Storage"
                }
            }
        }
        virtio {
            virtio0 {
                disk {
                    size               = 32
                    storage            = "VM-Storage"
                }
            }
            virtio1 {
                disk {
                    size               = 128
                    storage            = "VM-Storage"
                }
            }
        }
    }

    # VM Cloud-Init Settings
    os_type = "cloud-init"

    # (Optional) IP Address and Gateway
    ipconfig0 = "ip=192.168.178.13/24,gw=192.168.178.1"

    # (Optional) DNS Servers
    nameserver = "192.168.178.3"

    # (Optional) Default User
    ciuser = "soniiit"

    # (Optional) Add your Public SSH KEY
    sshkeys = "ssh-rsa  "
}

# k3s-soniiit-node03
resource "proxmox_vm_qemu" "k3s_soniiit_node03" {

    # VM General Settings
    target_node = "pve-srv-01"
    vmid = "0"
    name = "k3s-soniiit-node03"
    tags = "ubuntu,kubernetes"
    desc = "Node 04 Server for Kubernetes Cluster"
    automatic_reboot = false

    # The destination resource pool for the new VM
    pool = "K3s"

    # VM Advanced General Settings
    onboot = true
    boot = "order=virtio0;net0"
    bootdisk = "virtio"
    scsihw = "virtio-scsi-pci"

    # VM OS Settings
    full_clone = true
    clone = "ubuntu-server-24-04"
    bios = "seabios"

    # VM System Settings
    agent = 1

    # VM CPU Settings
    cores = 2
    sockets = 1
    cpu = "host"

    # VM Memory Settings
    memory = 16384

    # VM Network Settings
    network {
        bridge = "vmbr0"
        model  = "virtio"
    }

    # VM Disk Settings Cloud-Init
    disks {
        ide {
            ide0 {
                cloudinit {
                    storage = "VM-Storage"
                }
            }
        }
        virtio {
            virtio0 {
                disk {
                    size               = 32
                    storage            = "VM-Storage"
                }
            }
            virtio1 {
                disk {
                    size               = 128
                    storage            = "VM-Storage"
                }
            }
        }
    }

    # VM Cloud-Init Settings
    os_type = "cloud-init"

    # (Optional) IP Address and Gateway
    ipconfig0 = "ip=192.168.178.14/24,gw=192.168.178.1"

    # (Optional) DNS Servers
    nameserver = "192.168.178.3"

    # (Optional) Default User
    ciuser = "soniiit"

    # (Optional) Add your Public SSH KEY
    sshkeys = "ssh-rsa  "
}

