# resource "proxmox_vm_qemu" "k8s_lite_soniiit_cp01" {

#     # VM General Settings
#     target_node = "pve-srv-01"
#     vmid = "0"
#     name = "k8s-lite-soniiit-cp01"
#     tags = "ubuntu,kubernetes"
#     desc = "Control Plane Server 01 for Kubernetes Cluster"
#     automatic_reboot = false

#     # VM Advanced General Settings
#     onboot = true
#     boot = "order=virtio0;net0"
#     bootdisk = "virtio"
#     scsihw = "virtio-scsi-pci"

#     # VM OS Settings
#     full_clone = true
#     clone = "ubuntu-server-24-04"
#     bios = "seabios"

#     # VM System Settings
#     agent = 1

#     # VM CPU Settings
#     cores = 2
#     sockets = 1
#     cpu = "kvm64"

#     # VM Memory Settings
#     memory = 4096

#     # VM Network Settings
#     network {
#         bridge = "vmbr0"
#         model  = "virtio"
#     }

#     # VM Disk Settings Cloud-Init
#     disks {
#         ide {
#             ide0 {
#                 cloudinit {
#                     storage = "VM-Storage"
#                 }
#             }
#         }
#         virtio {
#             virtio0 {
#                 disk {
#                     size               = 32
#                     storage            = "VM-Storage"
#                 }
#             }
#         }
#     }

#     # VM Cloud-Init Settings
#     os_type = "cloud-init"

#     # (Optional) IP Address and Gateway
#     ipconfig0 = "ip=192.168.178.70/24,gw=192.168.178.1,ip6=dhcp"

#     # (Optional) DNS Servers
#     nameserver = "192.168.178.3"

#     # (Optional) Default User
#     ciuser = "soniiit"

#     # (Optional) Add your Public SSH KEY
#     sshkeys = "ssh-rsa  "
# }

# resource "proxmox_vm_qemu" "k8s_lite_soniiit_w01" {

#     # VM General Settings
#     target_node = "pve-srv-01"
#     vmid = "0"
#     name = "k8s-lite-soniiit-w01"
#     tags = "ubuntu,kubernetes"
#     desc = "Worker Server 01 for Kubernetes Cluster"
#     automatic_reboot = false

#     # VM Advanced General Settings
#     onboot = true
#     boot = "order=virtio0;net0"
#     bootdisk = "virtio"
#     scsihw = "virtio-scsi-pci"

#     # VM OS Settings
#     full_clone = true
#     clone = "ubuntu-server-24-04"
#     bios = "seabios"

#     # VM System Settings
#     agent = 1

#     # VM CPU Settings
#     cores = 2
#     sockets = 1
#     cpu = "kvm64"

#     # VM Memory Settings
#     memory = 8192

#     # VM Network Settings
#     network {
#         bridge = "vmbr0"
#         model  = "virtio"
#     }

#     # VM Disk Settings Cloud-Init
#     disks {
#         ide {
#             ide0 {
#                 cloudinit {
#                     storage = "VM-Storage"
#                 }
#             }
#         }
#         virtio {
#             virtio0 {
#                 disk {
#                     size               = 32
#                     storage            = "VM-Storage"
#                 }
#             }
#         }
#     }

#     # VM Cloud-Init Settings
#     os_type = "cloud-init"

#     # (Optional) IP Address and Gateway
#     ipconfig0 = "ip=192.168.178.71/24,gw=192.168.178.1,ip6=dhcp"

#     # (Optional) DNS Servers
#     nameserver = "192.168.178.3"

#     # (Optional) Default User
#     ciuser = "soniiit"

#     # (Optional) Add your Public SSH KEY
#     sshkeys = "ssh-rsa  "
# }

# resource "proxmox_vm_qemu" "k8s_lite_soniiit_w02" {

#     # VM General Settings
#     target_node = "pve-srv-01"
#     vmid = "0"
#     name = "k8s-lite-soniiit-w02"
#     tags = "ubuntu,kubernetes"
#     desc = "Worker Server 02 for Kubernetes Cluster"
#     automatic_reboot = false

#     # VM Advanced General Settings
#     onboot = true
#     boot = "order=virtio0;net0"
#     bootdisk = "virtio"
#     scsihw = "virtio-scsi-pci"

#     # VM OS Settings
#     full_clone = true
#     clone = "ubuntu-server-24-04"
#     bios = "seabios"

#     # VM System Settings
#     agent = 1

#     # VM CPU Settings
#     cores = 2
#     sockets = 1
#     cpu = "kvm64"

#     # VM Memory Settings
#     memory = 8192

#     # VM Network Settings
#     network {
#         bridge = "vmbr0"
#         model  = "virtio"
#     }

#     # VM Disk Settings Cloud-Init
#     disks {
#         ide {
#             ide0 {
#                 cloudinit {
#                     storage = "VM-Storage"
#                 }
#             }
#         }
#         virtio {
#             virtio0 {
#                 disk {
#                     size               = 32
#                     storage            = "VM-Storage"
#                 }
#             }
#         }
#     }

#     # VM Cloud-Init Settings
#     os_type = "cloud-init"

#     # (Optional) IP Address and Gateway
#     ipconfig0 = "ip=192.168.178.72/24,gw=192.168.178.1,ip6=dhcp"

#     # (Optional) DNS Servers
#     nameserver = "192.168.178.3"

#     # (Optional) Default User
#     ciuser = "soniiit"

#     # (Optional) Add your Public SSH KEY
#     sshkeys = "ssh-rsa  "
# }

# resource "proxmox_vm_qemu" "k8s_lite_soniiit_nfs01" {

#     # VM General Settings
#     target_node = "pve-srv-01"
#     vmid = "0"
#     name = "k8s-lite-soniiit-nfs01"
#     tags = "ubuntu,application"
#     desc = "NFS Server 01 for Kubernetes Cluster"
#     automatic_reboot = false

#     # VM Advanced General Settings
#     onboot = true
#     boot = "order=virtio0;net0"
#     bootdisk = "virtio"
#     scsihw = "virtio-scsi-pci"

#     # VM OS Settings
#     full_clone = true
#     clone = "ubuntu-server-24-04"
#     bios = "seabios"

#     # VM System Settings
#     agent = 1

#     # VM CPU Settings
#     cores = 1
#     sockets = 1
#     cpu = "kvm64"

#     # VM Memory Settings
#     memory = 2048

#     # VM Network Settings
#     network {
#         bridge = "vmbr0"
#         model  = "virtio"
#     }

#     # VM Disk Settings Cloud-Init
#     disks {
#         ide {
#             ide0 {
#                 cloudinit {
#                     storage = "VM-Storage"
#                 }
#             }
#         }
#         virtio {
#             virtio0 {
#                 disk {
#                     size               = 32
#                     storage            = "VM-Storage"
#                 }
#             }
#             virtio1 {
#                 disk {
#                     size               = 128
#                     storage            = "VM-Storage"
#                 }
#             }
#         }
#     }

#     # VM Cloud-Init Settings
#     os_type = "cloud-init"

#     # (Optional) IP Address and Gateway
#     ipconfig0 = "ip=192.168.178.73/24,gw=192.168.178.1,ip6=dhcp"

#     # (Optional) DNS Servers
#     nameserver = "192.168.178.3"

#     # (Optional) Default User
#     ciuser = "soniiit"

#     # (Optional) Add your Public SSH KEY
#     sshkeys = "ssh-rsa  "
# }